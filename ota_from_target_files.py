...
def PackRecoveryImages(output_zip, info_dict):
   ...
   if OPTIONS.device_type == "MMC":
     ...
     if hasattr(OPTIONS, "source_tmp"):
      source_recovery_img = common.GetBootableImage(
                             "recovery.img", "boot.img",
                             OPTIONS.source_tmp, "")
      source_recoveryfs_img = common.GetBootableImage(
                               "recoveryfs-unsparsed.img",
                               "recovery-unsparsed.ext4",
                               OPTIONS.source_tmp, "")

      if source_recovery_img.sha1 != target_recovery_img.sha1:
        common.ZipWriteStr(output_zip, "recoveryupgrade/" + target_recovery_img.name, target_recovery_img.data)
      else:
        print("Recovery Image is not changed, skip it")

      if source_recoveryfs_img.sha1 != target_recoveryfs_img.sha1:
        #TODO: RecoveryFS size always the same, need to find a way to check if RecoveryFs has changed
        #if source_recoveryfs_img.size != target_recoveryfs_img.size:
          common.ZipWriteStr(output_zip, "recoveryupgrade/" + target_recoveryfs_img.name, target_recoveryfs_img.data)
        #else:
        #  print("RecoveryFS has the same size but hash different, assume it's unchanged, skip it")
      else:
        print("RecoveryFS Image is not changed, skip it")
     else:
       common.ZipWriteStr(output_zip, "recoveryupgrade/" + target_recovery_img.name, target_recovery_img.data)
       common.ZipWriteStr(output_zip, "recoveryupgrade/" + target_recoveryfs_img.name, target_recoveryfs_img.data)
